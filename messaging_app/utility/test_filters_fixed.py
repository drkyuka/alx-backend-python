...
def test_message_content_filter(token, search_text="test"):
    ...
    # Since the serializer uses 'content' but the filter expects 'message_body',
    # we need to use the search functionality instead of the content filter
    ...
    if response.status_code == 200:
        ...
        if messages:
            for msg in messages:
                ...
                print_info(f"Message content: '{message_content}'")
                if search_text.lower() not in message_content.lower():
                    print_error(
